...
- A dynatrace API token having the right : `Ingest OpenTelemetry traces`, `ingest metrics'
...
```
export DT_TENANT_URL=<YOUR TENANT URL> (with https)
...
sed -i "s,DT_API_TOKEN_TO_REPLACE,$DT_API_TOKEN," otelemetry/openTelemetry.yaml
```
...
```
kubectl label namespaces default linkerd.io/inject=enabled
```
### 9. Deploy the hipster-shop
```
kubectl create ns hipster-shop
kubectl label namespaces hipster-shop linkerd.io/inject=enabled
